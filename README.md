...
<head>
    ...
    <link rel="preload" as="style" href="https://fonts.googleapis.com/css2?family=Philosopher:wght@400;700&family=Noto+Serif+TC:wght@400;700&family=Noto+Sans+Symbols+2&display=swap">
    <link href="https://fonts.googleapis.com/css2?family=Philosopher:wght@400;700&family=Noto+Serif+TC:wght@400;700&family=Noto+Sans+Symbols+2&display=swap" rel="stylesheet">
    ...
    <style>
        ...
        * {
        ...
        #intro-title {
            font-size: clamp(1.0em, 6vw, 1.6em);
            white-space: nowrap;
            overflow: visible;
        }

        /* Allow line breaks within intro title */
        #intro-title br {
            display: block;
        }
        ...
        .gallery-container {
            ...
            margin: 10px auto 30px; /* Add vertical margins for scrolling space */
            background: linear-gradient(135deg, rgba(26, 13, 46, 0.98) 0%, rgba(106, 5, 114, 0.95) 100%);
            border-radius: 15px;
            padding: 50px 18px 40px; /* Extra bottom padding for mobile */
            border: none;
            border-left: 3px solid var(--color-secondary);
            border-right: 3px solid var(--color-secondary);
            box-shadow: 0 0 80px rgba(212, 175, 55, 0.5);
            position: relative;
            overflow: visible; /* No scrolling on container, modal handles it */
        }
        ...
        .gallery-grid {
            ...
            gap: 10px;
            margin-top: 10px;
            padding-bottom: 20px; /* Extra padding for button visibility */
        ...
        /* Gallery action buttons - styled as grid items */
        .gallery-button-item {
            background: transparent; /* Remove outer background */
            border-radius: 8px;
            padding: 5px; /* Minimal padding */
            border: none; /* Remove outer border - only keep inner button border */
            display: flex;
            align-items: center;
            justify-content: center;
            transition: all 0.3s ease;
        ...
        .gallery-button-item:hover {
            transform: translateY(-4px);
        }

        .gallery-button-item button {
            padding: 12px 16px;
            font-size: 0.85em;
            width: 100%;
            white-space: normal;
        ...
        @media (max-width: 768px) {
            #intro-title {
                font-size: clamp(1.1em, 7vw, 1.8em);
            }
            ...
            /* Ensure result title is centered on mobile, not affected by lang-toggle button */
            #result-title {
            ...
            .gallery-container {
                padding: 55px 15px 50px; /* Extra bottom padding for mobile scrolling */
                margin: 10px auto 40px; /* More bottom margin on mobile */
            }
        ...
        @media (max-width: 520px) {
            ...
            .gallery-container {
                ...
                margin: 8px auto 50px; /* More bottom margin on small screens */
            }
...
<body>
    ...
    <div id="result-page" class="container">
        <div style="margin-bottom: 10px; text-align: center; width: 100%; padding: 0 0 0 0; box-sizing: border-box;">
            <h2 id="result-title" style="margin: 0 auto; font-size: 1.3em; text-align: center; padding: 0; width: 100%; display: block; box-sizing: border-box;">你的專屬塔羅啟示</h2>
        </div>
    ...
    <script>
        ...
        const questionsData = {
            zh: [
                ...
                { question: "在神秘的桌子上, 你想拿起哪一樣物品來引導你?", options: [{ text: "清澈的水晶球, 觀看未來的影像", value: 0 }, { text: "古老的鑰匙, 開啟被塵封的門", value: 4 }, { text: "發光的飾物, 朋友贈送的心意", value: 8 }, { text: "純白的羽毛, 象徵輕盈與自由", value: 12 }] },
        ...
        function openGallery() {
            ...
            // Add action buttons as grid items in the empty spaces (positions 22 and 23)
            // Remove any existing button items first
            const existingButtons = grid.querySelectorAll('.gallery-button-item');
            existingButtons.forEach(btn => btn.remove());

            if (unlockedCount > 0) {
                // Add Save Collection button (always shown when at least 1 card unlocked)
                const saveButtonItem = document.createElement('div');
                saveButtonItem.className = 'gallery-button-item';
                saveButtonItem.innerHTML = `
                    <button class="btn" onclick="saveGalleryCollection()">${lang.btnSaveCollection}</button>
                `;
                grid.appendChild(saveButtonItem);

                // Add Replay Animation button (only shown when all cards unlocked)
                if (allUnlocked) {
                    const replayButtonItem = document.createElement('div');
                    replayButtonItem.className = 'gallery-button-item';
                    replayButtonItem.innerHTML = `
                        <button class="btn" onclick="replayCelebration()">${lang.btnReplayAnimation}</button>
                    `;
                    grid.appendChild(replayButtonItem);
                }
            }
        ...
        async function drawQRCode(ctx, canvas) {
            try {
                ...
                // Wait for QR image to fully load (Samsung browser fix)
                ...
                if (qrImg && qrImg.complete && qrImg.naturalWidth > 0) {
                    ...
                    // Draw QR image with Samsung browser compatibility
                    ctx.save();
                    roundRect(ctx, qrX + qrPadding, qrY + qrPadding, qrSize - qrPadding * 2, qrSize - qrPadding * 2, qrRadius - 5);
                    ctx.clip();
                    // Force image smoothing off for crisp QR code
    ...
    // Draw card image if loaded (moved down together with heading and name)
        ...
        // Hide loading screen with coordinated crossfade animation
        function hideLoadingScreen() {
            const loadingScreen = document.getElementById('loading-screen');
            const loadingSymbol = document.getElementById('loading-symbol');
            const progressContainer = document.getElementById('loading-progress-container');

            // Add ready class to start the 3-spin + enlarge + fade animation
            if (loadingSymbol) {
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // DON'T show intro page yet - wait for loading screen to finish
            // showPage('intro-page') is now called AFTER loading screen completes
            checkFileSharing();
            ...
            preloadTarotImages();
            // Mark images as started after minimum display time for smooth UX
            setTimeout(() => {
                imagesStarted = true;
